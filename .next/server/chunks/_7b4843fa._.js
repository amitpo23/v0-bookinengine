...
רוצה לבדוק? שאל אותי שאלה והראה לך איך אני משתמש בהנחיות החדשות! 🚀`,sessionId:f,adminMode:!0,customInstructions:e})}}let v="he"===c,w=o?.name||"Dizengoff Inn",S=o?.apiSettings?.mediciHotelName||"Dizengoff Inn",I=o?.apiSettings?.mediciCity||o?.city||"Tel Aviv";console.log("[v0] Chat request - Hotel:",w,"API Name:",S),console.log("[v0] Session ID:",f),console.log("[v0] Admin Mode:",l.ChatMemoryService.isAdminMode(f)),console.log("[v0] Booking state:",p);let R=new Date().toISOString().split("T")[0],A=(0,s.getBookingAgentPrompt)(c,w,I,R,o?.id,o?.knowledgeBase,o?.systemInstructions),k=l.ChatMemoryService.getCustomInstructions(f);k&&(A+=`
...
Which room interests you? Write the number or "I want room number X"`),bookingData:{type:"search_results",data:{rooms:a,searchContext:{dateFrom:e.dateFrom,dateTo:e.dateTo,adults:e.adults||2,children:e.children||[],city:e.city||I},jsonRequest:s}}})}}catch(t){console.error("[v0] Search error:",t),i.SearchLogger.logSearch({hotelName:S,city:searchParams?.city||I,dateFrom:searchParams?.dateFrom,dateTo:searchParams?.dateTo,adults:searchParams?.adults||2,children:searchParams?.children||0,resultsCount:0,success:!1,errorMessage:t instanceof Error?t.message:"Unknown search error",source:"chat",channel:"web",metadata:{hotelId:o?.id,error:t instanceof Error?t.toString():String(t)}}).catch(e=>console.error("Failed to log search error:",e));let e=E.replace(/\[SEARCH\].*?\[\/SEARCH\]/s,"").trim();return Response.json({message:e+"\n\n"+(v?"הייתה בעיה ביצירת קשר עם מערכת ההזמנות. אנא נסה שוב.":"There was an issue contacting the booking system. Please try again.")})}}let x=E.match(/\[SELECT_ROOM\](.*?)\[\/SELECT_ROOM\]/s);if(x&&p?.jsonRequest){console.log("[v0] Room selected, calling PreBook...");try{let e=JSON.parse(x[1]),t=await h({code:e.code,hotelId:e.hotelId,dateFrom:p.searchContext.dateFrom,dateTo:p.searchContext.dateTo,adults:p.searchContext.adults,children:p.searchContext.children,searchRequestJson:p.jsonRequest}),o=E.replace(/\[SELECT_ROOM\].*?\[\/SELECT_ROOM\]/s,"").trim();return Response.json({message:o+"\n\n"+(v?`מעולה! שמרתי את החדר עבורך.
...
- Full name
- Email
- Phone number`),bookingData:{type:"prebook_complete",data:{preBookData:t,selectedRoom:e}}})}catch(e){return console.error("[v0] PreBook error:",e),Response.json({message:v?"הייתה בעיה בשמירת החדר. אנא נסה שוב או בחר חדר אחר.":"There was an issue reserving the room. Please try again or select another room."})}}let D=E.match(/\[BOOK\](.*?)\[\/BOOK\]/s);if(D&&p?.preBookData&&p?.selectedRoom&&p?.jsonRequest){console.log("[v0] Completing booking...");try{let e=JSON.parse(D[1]),t=await m({token:p.preBookData.token,preBookId:p.preBookData.preBookId,customer:e,searchRequestJson:p.jsonRequest,roomCode:p.selectedRoom.code,adults:p.searchContext?.adults||2,children:p.searchContext?.children||[]});if(t.bookingId&&t.supplierReference&&r.emailService.isEnabled())try{let o=JSON.parse(p.jsonRequest),a=o.dates?.from||new Date().toISOString(),s=o.dates?.to||new Date().toISOString(),i=Math.ceil((new Date(s).getTime()-new Date(a).getTime())/864e5);r.emailService.sendBookingConfirmation({to:e.email,customerName:`${e.firstName} ${e.lastName}`,bookingId:t.bookingId,supplierReference:t.supplierReference,hotelName:p.selectedRoom?.hotelName||w,roomType:p.selectedRoom?.roomName||"Room",checkIn:(0,n.format)(new Date(a),"MMM dd, yyyy"),checkOut:(0,n.format)(new Date(s),"MMM dd, yyyy"),nights:i,adults:p.searchContext?.adults||2,children:p.searchContext?.children?.length||0,totalPrice:p.preBookData?.netPrice||0,currency:o.currencies?.[0]||"USD",language:c}).then(e=>{e.success?console.log("[AI Chat] ✅ Confirmation email sent",{bookingId:t.bookingId,emailId:e.emailId}):console.warn("[AI Chat] ⚠️ Email failed (non-critical)",e.error)}).catch(e=>{console.error("[AI Chat] Email error (non-critical):",e)})}catch(e){console.error("[AI Chat] Failed to parse search request for email:",e)}let o=E.replace(/\[BOOK\].*?\[\/BOOK\]/s,"").trim();return Response.json({message:o+"\n\n"+(v?`🎉 ההזמנה הושלמה בהצלחה!
