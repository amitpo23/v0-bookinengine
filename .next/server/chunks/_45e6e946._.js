...
`;e.s(["scarletKnowledgeBase",0,o,"scarletSystemInstructions",0,a],304101)},373424,e=>{"use strict";var t=e.i(304101);let o=`אתה סוכן הזמנות AI מקצועי עבור פלטפורמת SaaS להזמנות מלונות הרצה בתוך אפליקציית Vercel.
...
- תוך כיבוד קפדני של תגובות ה-API וכללי המלון.`,a=`You are the AI Hotel Booking Agent for the hotel booking SaaS platform running inside a Vercel-based application.
...
שאלות נפוצות:
${t.scarletKnowledgeBase.faq.map(e=>`ש: ${e.q}
ת: ${e.a}`).join("\n\n")}
...
Remember: Use city OR hotelName, never both together!`;return("he"===e?o:a)+d}e.s(["getBookingAgentPrompt",()=>s])},765607,e=>{"use strict";var t=e.i(224389);let o=process.env.NEXT_PUBLIC_SUPABASE_URL||"",a=process.env.SUPABASE_SERVICE_ROLE_KEY||process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY||"",s=null;o&&a&&(s=(0,t.createClient)(o,a));class r{static async logSearch(e){if(!s)return void console.log("[SearchLogger] Skipping log - Supabase not configured");try{let t=e.dateFrom instanceof Date?e.dateFrom.toISOString().split("T")[0]:e.dateFrom,o=e.dateTo instanceof Date?e.dateTo.toISOString().split("T")[0]:e.dateTo,{error:a}=await s.from("search_logs").insert([{hotel_id:e.hotelId,search_query:e.searchQuery,destination:e.destination,hotel_name:e.hotelName,city:e.city,date_from:t,date_to:o,adults:e.adults,children:e.children,results_count:e.resultsCount??0,found_hotels:e.foundHotels,found_rooms:e.foundRooms,duration_ms:e.durationMs,success:e.success??!0,error_message:e.errorMessage,user_id:e.userId,session_id:e.sessionId,ip_address:e.ipAddress,user_agent:e.userAgent,source:e.source,channel:e.channel,metadata:e.metadata}]);a&&console.error("[SearchLogger] Supabase insert error:",a)}catch(e){console.error("[SearchLogger] Error logging search:",e)}}static async getSearchLogs(e){if(!s)return console.log("[SearchLogger] Skipping getLogs - Supabase not configured"),{logs:[],total:0,limit:e?.limit??50,offset:e?.offset??0};let t=e?.limit??50,o=e?.offset??0,a=s.from("search_logs").select("*",{count:"exact"}).order("created_at",{ascending:!1}).range(o,o+t-1);e?.hotelId&&(a=a.eq("hotel_id",e.hotelId)),e?.userId&&(a=a.eq("user_id",e.userId)),e?.success!==void 0&&(a=a.eq("success",e.success)),e?.source&&(a=a.eq("source",e.source)),e?.dateFrom&&(a=a.gte("created_at",e.dateFrom.toISOString())),e?.dateTo&&(a=a.lte("created_at",e.dateTo.toISOString()));let{data:r,count:i,error:n}=await a;return n?(console.error("[SearchLogger] Error fetching logs:",n),{logs:[],total:0,limit:t,offset:o}):{logs:r||[],total:i||0,limit:t,offset:o}}static async getSearchStats(e){if(!s)return console.log("[SearchLogger] Skipping getSearchStats - Supabase not configured"),{totalSearches:0,successfulSearches:0,failedSearches:0,avgDurationMs:0};let t=s.from("search_logs").select("*",{count:"exact",head:!0}),o=s.from("search_logs").select("*",{count:"exact",head:!0}).eq("success",!0);if(e?.hotelId&&(t=t.eq("hotel_id",e.hotelId),o=o.eq("hotel_id",e.hotelId)),e?.dateFrom){let a=e.dateFrom.toISOString();t=t.gte("created_at",a),o=o.gte("created_at",a)}if(e?.dateTo){let a=e.dateTo.toISOString();t=t.lte("created_at",a),o=o.lte("created_at",a)}let{count:a}=await t,{count:r}=await o;return{total:a||0,successful:r||0,failed:(a||0)-(r||0),successRate:(a||0)>0?((r||0)/a*100).toFixed(2)+"%":"0%",bySource:{},byDestination:{}}}static async deleteOldLogs(e=30){let t=new Date;t.setDate(t.getDate()-e);let{data:o,error:a}=await s.from("search_logs").delete().lt("created_at",t.toISOString());return a?(console.error("[SearchLogger] Error deleting old logs:",a),0):o?.length||0}static async deleteLog(e){try{let{error:t}=await s.from("search_logs").delete().eq("id",e);if(t)return console.error("[SearchLogger] Error deleting log:",t),!1;return!0}catch(e){return console.error("[SearchLogger] Error deleting log:",e),!1}}}e.s(["SearchLogger",()=>r])},646958,e=>{"use strict";let t=new Map,o=["זה מנהל המלון","אני מנהל המלון","זה האדמין","אני האדמין","admin mode","manager mode"],a=["הנחיות חדשות","עדכן הנחיות","שנה הנחיות","הגדרות חדשות","new instructions","update instructions"];class s{static getOrCreateMemory(e,o){if(!t.has(e)){let a={sessionId:e,hotelId:o,messages:[],context:{},metadata:{startedAt:Date.now(),lastActivityAt:Date.now(),messageCount:0}};t.set(e,a),console.log(`[Memory] Created new memory for session ${e}`)}return t.get(e)}static addMessage(e,o,a){let s=t.get(e);s&&(s.messages.push({role:o,content:a,timestamp:Date.now()}),s.metadata.lastActivityAt=Date.now(),s.metadata.messageCount++,s.messages.length>50&&(s.messages=s.messages.slice(-50)),console.log(`[Memory] Added ${o} message to session ${e} (total: ${s.messages.length})`))}static isAdminTrigger(e){let t=e.toLowerCase().trim();return o.some(e=>t.includes(e.toLowerCase()))}static activateAdminMode(e){let o=t.get(e);o&&(o.context.isAdminMode=!0,console.log(`[Memory] 🔐 Admin mode ACTIVATED for session ${e}`))}static isAdminMode(e){let o=t.get(e);return o?.context.isAdminMode===!0}static extractAndSaveInstructions(e,o){let s=t.get(e);if(!s||!s.context.isAdminMode||!a.some(e=>o.includes(e)))return null;let r="";for(let e of a)if(o.includes(e)){let t=o.split(e);if(t.length>1){r=t[1].trim();break}}return r?(s.context.customInstructions=r,console.log(`[Memory] 📝 Saved custom instructions for session ${e}:`,r.substring(0,100)+"..."),r):null}static getCustomInstructions(e){let o=t.get(e);return o?.context.customInstructions||null}static getFormattedHistory(e,o=10){let a=t.get(e);return a?a.messages.filter(e=>"system"!==e.role).slice(-o):[]}static getSummary(e){let o=t.get(e);if(!o||0===o.messages.length)return"שיחה חדשה - אין היסטוריה קודמת.";let a=o.messages.filter(e=>"user"===e.role).length,s=o.messages.filter(e=>"assistant"===e.role).length,r=o.context.userPreferences,i=`היסטוריית שיחה: ${a} הודעות ממשתמש, ${s} תגובות.
...
מה תרצה לעדכן?`,sessionId:y,adminMode:!0});if(l.ChatMemoryService.isAdminMode(y)){let e=l.ChatMemoryService.extractAndSaveInstructions(y,v.content);if(e)return Response.json({text:`✅ **הנחיות עודכנו בהצלחה!**
...
רוצה לבדוק? שאל אותי שאלה והראה לך איך אני משתמש בהנחיות החדשות! 🚀`,sessionId:y,adminMode:!0,customInstructions:e})}}let w="he"===a,S=o?.name||"Dizengoff Inn",I=o?.apiSettings?.mediciHotelName||"Dizengoff Inn",R=o?.apiSettings?.mediciCity||o?.city||"Tel Aviv";console.log("[v0] Chat request - Hotel:",S,"API Name:",I),console.log("[v0] Session ID:",y),console.log("[v0] Admin Mode:",l.ChatMemoryService.isAdminMode(y)),console.log("[v0] Booking state:",d);let A=new Date().toISOString().split("T")[0],k=(0,s.getBookingAgentPrompt)(a,S,R,A,o?.id,o?.knowledgeBase,o?.systemInstructions);if(o?.id==="scarlet-hotel"){let e=c.scarletKnowledgeBase;k+=`

## 🏨 מידע מפורט על מלון סקרלט תל אביב:

`,k+=`**זהות המלון:**
`,k+=`- שם: ${e.identity.name} (${e.identity.englishName})
`,k+=`- סלוגן: ${e.identity.tagline}
`,k+=`- קונצפט: ${e.identity.concept}
`,k+=`- קהל יעד: ${e.identity.targetAudience}

`,k+=`**נקודות מכירה ייחודיות:**
`,e.identity.uniqueSellingPoints.forEach(e=>{k+=`- ${e}
`}),k+=`
`,k+=`**מיקום:**
`,k+=`- כתובת: ${e.location.address}
`,k+=`- שכונה: ${e.location.neighborhood}
`,k+=`- אטרקציות בקרבת מקום:
...
`,k+=`- גודל: ${e.size}
`,k+=`- תיאור: ${e.description}
`,k+=`- מקסימום אורחים: ${e.maxGuests}
`,k+=`- טווח מחירים: ${e.priceRange}
`,e.detailedFeatures&&(k+=`- מיטה: ${e.detailedFeatures.bedding}
`,k+=`- חדר רחצה: ${e.detailedFeatures.bathroom}
`,k+=`- נוף: ${e.detailedFeatures.view}
`)}),k+=`
`,k+=`**שירותים:**
`,k+=`- דלפק קבלה: ${e.services.frontDesk.hours}
`,k+=`- שפות: ${e.services.frontDesk.languages.join(", ")}
`,k+=`- שירותים מיוחדים: ${e.services.specialServices.join(", ")}

`,k+=`**מדיניות:**
`,k+=`- צ'ק-אין: ${e.policies.checkIn}
`,k+=`- צ'ק-אאוט: ${e.policies.checkOut}
`,k+=`- ביטול: ${e.policies.cancellation.join("; ")}
...
`,k+=`**טיפים חשובים:**
`,e.agentInstructions.keyTips.forEach(e=>{k+=`✨ ${e}
`}),console.log("[Scarlet] Enhanced system prompt with full knowledge base")}let E=l.ChatMemoryService.getCustomInstructions(y);E&&(k+=`

## 🔐 הנחיות מותאמות אישית ממנהל המלון (עדיפות גבוהה!):
${E}

הנחיות אלה חשובות ביותר ודורסות הנחיות אחרות במקרה של סתירה.`,console.log("[v0] Applied custom instructions from admin"));let C=l.ChatMemoryService.getSummary(y);k+=`

## 💬 הקשר שיחה:
${C}`,console.log("[v0] Calling AI model (Groq)...");let O=await f.chat.completions.create({model:"llama-3.3-70b-versatile",messages:[{role:"system",content:k},...t.map(e=>({role:e.role,content:e.content}))],temperature:.7,max_tokens:2048}),x=O.choices[0]?.message?.content||"";console.log("[v0] AI response:",x.slice(0,500)),l.ChatMemoryService.addMessage(y,"assistant",x);let D=x.match(/\[SEARCH\](.*?)\[\/SEARCH\]/s);if(D){console.log("[v0] Found search request:",D[1]);try{let e=JSON.parse(D[1]);console.log("[v0] Parsed search params:",e);if(o?.id==="scarlet-hotel")try{let e=await fetch("https://static-data.innstant-servers.com/hotels/863233",{headers:{Authorization:"Bearer $2y$10$yWot7dUYoc7.viH8vK1s0OG.D0n5uKm19Z84WznDiB.ESBnPOikr6"}});if(e.ok){let t=await e.json();console.log("[Scarlet] Static Data API response:",t),t.rooms}}catch(e){console.log("[Scarlet] Static Data API error:",e)}let{results:a,jsonRequest:s}=await h({hotelName:I,city:e.city||R,dateFrom:e.dateFrom,dateTo:e.dateTo,adults:e.adults||2,children:e.children||[],filterHotelId:o?.id==="scarlet-hotel"?"863233":void 0}),r=[];if(a?.items&&Array.isArray(a.items)?r=a.items:Array.isArray(a)?r=a:a?.hotels&&(r=a.hotels),r.length>0){let a=r.slice(0,6).map((t,o)=>{let a=t.price?.amount||t.netPrice?.amount||t.price||0,s=t.price?.currency||t.netPrice?.currency||"USD",r=t.images||[],i=function(e){if(!e||0===e.length)return"";let t=e.find(e=>"mainimage"===e.title);return t?u(t):u(e[0])}(r),n=r&&0!==r.length?r.slice(0,10).map(e=>u(e)).filter(Boolean):[],l=t.facilities?.tags||t.facilities?.list||[];return{code:t.code||`${t.hotelId||S}:${o}:${Date.now()}`,hotelId:t.hotelId||0,name:t.hotelName||t.name||"Hotel",hotelName:t.hotelName||S,roomType:t.name||t.category||"Standard Room",board:t.board||"RO",price:a,currency:s,cancellation:t.cancellation?.type||"non-refundable",confirmation:t.confirmation||"immediate",image:i,images:n,description:t.description||"",facilities:l,location:t.city||e.city||R,address:t.address||"",rating:t.stars||4}}),n=x.replace(/\[SEARCH\].*?\[\/SEARCH\]/s,"").trim(),l=a.map((e,t)=>{let o=0===t?w?" (הכי משתלם)":" (Best Value)":t===a.length-1?w?" (הכי גמיש)":" (Most Flexible)":"";return w?`${t+1}. ${e.hotelName} - ${e.roomType}${o}
...
Which room interests you? Write the number or "I want room number X"`),bookingData:{type:"search_results",data:{rooms:a,searchContext:{dateFrom:e.dateFrom,dateTo:e.dateTo,adults:e.adults||2,children:e.children||[],city:e.city||R},jsonRequest:s}}})}}catch(t){console.error("[v0] Search error:",t),i.SearchLogger.logSearch({hotelName:I,city:searchParams?.city||R,dateFrom:searchParams?.dateFrom,dateTo:searchParams?.dateTo,adults:searchParams?.adults||2,children:searchParams?.children||0,resultsCount:0,success:!1,errorMessage:t instanceof Error?t.message:"Unknown search error",source:"chat",channel:"web",metadata:{hotelId:o?.id,error:t instanceof Error?t.toString():String(t)}}).catch(e=>console.error("Failed to log search error:",e));let e=x.replace(/\[SEARCH\].*?\[\/SEARCH\]/s,"").trim();return Response.json({message:e+"\n\n"+(w?"הייתה בעיה ביצירת קשר עם מערכת ההזמנות. אנא נסה שוב.":"There was an issue contacting the booking system. Please try again.")})}}let j=x.match(/\[SELECT_ROOM\](.*?)\[\/SELECT_ROOM\]/s);if(j&&d?.jsonRequest){console.log("[v0] Room selected, calling PreBook...");try{let e=JSON.parse(j[1]),t=await m({code:e.code,hotelId:e.hotelId,dateFrom:d.searchContext.dateFrom,dateTo:d.searchContext.dateTo,adults:d.searchContext.adults,children:d.searchContext.children,searchRequestJson:d.jsonRequest}),o=x.replace(/\[SELECT_ROOM\].*?\[\/SELECT_ROOM\]/s,"").trim();return Response.json({message:o+"\n\n"+(w?`מעולה! שמרתי את החדר עבורך.
...
- Full name
- Email
- Phone number`),bookingData:{type:"prebook_complete",data:{preBookData:t,selectedRoom:e}}})}catch(e){return console.error("[v0] PreBook error:",e),Response.json({message:w?"הייתה בעיה בשמירת החדר. אנא נסה שוב או בחר חדר אחר.":"There was an issue reserving the room. Please try again or select another room."})}}let T=x.match(/\[BOOK\](.*?)\[\/BOOK\]/s);if(T&&d?.preBookData&&d?.selectedRoom&&d?.jsonRequest){console.log("[v0] Completing booking...");try{let e=JSON.parse(T[1]),t=await p({token:d.preBookData.token,preBookId:d.preBookData.preBookId,customer:e,searchRequestJson:d.jsonRequest,roomCode:d.selectedRoom.code,adults:d.searchContext?.adults||2,children:d.searchContext?.children||[]});if(t.bookingId&&t.supplierReference&&r.emailService.isEnabled())try{let o=JSON.parse(d.jsonRequest),s=o.dates?.from||new Date().toISOString(),i=o.dates?.to||new Date().toISOString(),l=Math.ceil((new Date(i).getTime()-new Date(s).getTime())/864e5);r.emailService.sendBookingConfirmation({to:e.email,customerName:`${e.firstName} ${e.lastName}`,bookingId:t.bookingId,supplierReference:t.supplierReference,hotelName:d.selectedRoom?.hotelName||S,roomType:d.selectedRoom?.roomName||"Room",checkIn:(0,n.format)(new Date(s),"MMM dd, yyyy"),checkOut:(0,n.format)(new Date(i),"MMM dd, yyyy"),nights:l,adults:d.searchContext?.adults||2,children:d.searchContext?.children?.length||0,totalPrice:d.preBookData?.netPrice||0,currency:o.currencies?.[0]||"USD",language:a}).then(e=>{e.success?console.log("[AI Chat] ✅ Confirmation email sent",{bookingId:t.bookingId,emailId:e.emailId}):console.warn("[AI Chat] ⚠️ Email failed (non-critical)",e.error)}).catch(e=>{console.error("[AI Chat] Email error (non-critical):",e)})}catch(e){console.error("[AI Chat] Failed to parse search request for email:",e)}let o=x.replace(/\[BOOK\].*?\[\/BOOK\]/s,"").trim();return Response.json({message:o+"\n\n"+(w?`🎉 ההזמנה הושלמה בהצלחה!
...
You've received confirmation at ${e.email}`),bookingData:{type:"booking_complete",data:{bookingId:t.bookingId,supplierReference:t.supplierReference}}})}catch(e){return console.error("[v0] Book error:",e),Response.json({message:w?"הייתה בעיה בסיום ההזמנה. אנא נסה שוב או צור קשר עם התמיכה.":"There was an issue completing the booking. Please try again or contact support."})}}let M=x.replace(/\[SEARCH\].*?\[\/SEARCH\]/s,"").replace(/\[SELECT_ROOM\].*?\[\/SELECT_ROOM\]/s,"").replace(/\[BOOK\].*?\[\/BOOK\]/s,"").trim();return Response.json({message:M,sessionId:y,hasMemory:b.messages.length>0,messageCount:b.metadata.messageCount,adminMode:l.ChatMemoryService.isAdminMode(y)})}catch(e){return console.error("[v0] AI Chat error:",e),Response.json({message:"Sorry, an error occurred. Please try again."},{status:500})}}process.env.TAVILY_API_KEY,e.s(["POST",()=>g]),o()}catch(e){o(e)}},!1),991683,e=>e.a(async(t,o)=>{try{var a=e.i(747909),s=e.i(174017),r=e.i(996250),i=e.i(759756),n=e.i(561916),l=e.i(114444),c=e.i(837092),d=e.i(869741),u=e.i(316795),h=e.i(487718),m=e.i(995169),p=e.i(47587),g=e.i(666012),f=e.i(570101),y=e.i(626937),b=e.i(10372),v=e.i(193695);e.i(52474);var w=e.i(257297),S=e.i(326259),I=t([S]);[S]=I.then?(await I)():I;let k=new a.AppRouteRouteModule({definition:{kind:s.RouteKind.APP_ROUTE,page:"/api/ai/booking-chat/route",pathname:"/api/ai/booking-chat",filename:"route",bundlePath:""},distDir:".next",relativeProjectDir:"",resolvedPagePath:"[project]/app/api/ai/booking-chat/route.ts",nextConfigOutput:"",userland:S}),{workAsyncStorage:E,workUnitAsyncStorage:C,serverHooks:O}=k;function R(){return(0,r.patchFetch)({workAsyncStorage:E,workUnitAsyncStorage:C})}async function A(e,t,o){k.isDev&&(0,i.addRequestMeta)(e,"devRequestTimingInternalsEnd",process.hrtime.bigint());let a="/api/ai/booking-chat/route";a=a.replace(/\/index$/,"")||"/";let r=await k.prepare(e,t,{srcPage:a,multiZoneDraftMode:!1});if(!r)return t.statusCode=400,t.end("Bad Request"),null==o.waitUntil||o.waitUntil.call(o,Promise.resolve()),null;let{buildId:S,params:I,nextConfig:R,parsedUrl:A,isDraftMode:E,prerenderManifest:C,routerServerContext:O,isOnDemandRevalidate:x,revalidateOnlyGenerated:D,resolvedPathname:j,clientReferenceManifest:T,serverActionsManifest:M}=r,P=(0,d.normalizeAppPath)(a),$=!!(C.dynamicRoutes[P]||C.routes[j]),N=async()=>((null==O?void 0:O.render404)?await O.render404(e,t,A,!1):t.end("This page could not be found"),null);if($&&!E){let e=!!C.routes[j],t=C.dynamicRoutes[P];if(t&&!1===t.fallback&&!e){if(R.experimental.adapterPath)return await N();throw new v.NoFallbackError}}let B=null;!$||k.isDev||E||(B=j,B="/index"===B?"/":B);let _=!0===k.isDev||!$,H=$&&!_;M&&T&&(0,l.setReferenceManifestsSingleton)({page:a,clientReferenceManifest:T,serverActionsManifest:M,serverModuleMap:(0,c.createServerModuleMap)({serverActionsManifest:M})});let F=e.method||"GET",z=(0,n.getTracer)(),Y=z.getActiveScopeSpan(),L={params:I,prerenderManifest:C,renderOpts:{experimental:{authInterrupts:!!R.experimental.authInterrupts},cacheComponents:!!R.cacheComponents,supportsDynamicResponse:_,incrementalCache:(0,i.getRequestMeta)(e,"incrementalCache"),cacheLifeProfiles:R.cacheLife,waitUntil:o.waitUntil,onClose:e=>{t.on("close",e)},onAfterTaskError:void 0,onInstrumentationRequestError:(t,o,a)=>k.onRequestError(e,t,a,O)},sharedContext:{buildId:S}},q=new u.NodeNextRequest(e),U=new u.NodeNextResponse(t),K=h.NextRequestAdapter.fromNodeNextRequest(q,(0,h.signalFromNodeResponse)(t));try{let r=async e=>k.handle(K,L).finally(()=>{if(!e)return;e.setAttributes({"http.status_code":t.statusCode,"next.rsc":!1});let o=z.getRootSpanAttributes();if(!o)return;if(o.get("next.span_type")!==m.BaseServerSpan.handleRequest)return void console.warn(`Unexpected root span type '${o.get("next.span_type")}'. Please report this Next.js issue https://github.com/vercel/next.js`);let s=o.get("next.route");if(s){let t=`${F} ${s}`;e.setAttributes({"next.route":s,"http.route":s,"next.span_name":t}),e.updateName(t)}else e.updateName(`${F} ${a}`)}),l=!!(0,i.getRequestMeta)(e,"minimalMode"),c=async i=>{var n,c;let d=async({previousCacheEntry:s})=>{try{if(!l&&x&&D&&!s)return t.statusCode=404,t.setHeader("x-nextjs-cache","REVALIDATED"),t.end("This page could not be found"),null;let a=await r(i);e.fetchMetrics=L.renderOpts.fetchMetrics;let n=L.renderOpts.pendingWaitUntil;n&&o.waitUntil&&(o.waitUntil(n),n=void 0);let c=L.renderOpts.collectedTags;if(!$)return await (0,g.sendResponse)(q,U,a,L.renderOpts.pendingWaitUntil),null;{let e=await a.blob(),t=(0,f.toNodeOutgoingHttpHeaders)(a.headers);c&&(t[b.NEXT_CACHE_TAGS_HEADER]=c),!t["content-type"]&&e.type&&(t["content-type"]=e.type);let o=void 0!==L.renderOpts.collectedRevalidate&&!(L.renderOpts.collectedRevalidate>=b.INFINITE_CACHE)&&L.renderOpts.collectedRevalidate,s=void 0===L.renderOpts.collectedExpire||L.renderOpts.collectedExpire>=b.INFINITE_CACHE?void 0:L.renderOpts.collectedExpire;return{value:{kind:w.CachedRouteKind.APP_ROUTE,status:a.status,body:Buffer.from(await e.arrayBuffer()),headers:t},cacheControl:{revalidate:o,expire:s}}}}catch(t){throw(null==s?void 0:s.isStale)&&await k.onRequestError(e,t,{routerKind:"App Router",routePath:a,routeType:"route",revalidateReason:(0,p.getRevalidateReason)({isStaticGeneration:H,isOnDemandRevalidate:x})},O),t}},u=await k.handleResponse({req:e,nextConfig:R,cacheKey:B,routeKind:s.RouteKind.APP_ROUTE,isFallback:!1,prerenderManifest:C,isRoutePPREnabled:!1,isOnDemandRevalidate:x,revalidateOnlyGenerated:D,responseGenerator:d,waitUntil:o.waitUntil,isMinimalMode:l});if(!$)return null;if((null==u||null==(n=u.value)?void 0:n.kind)!==w.CachedRouteKind.APP_ROUTE)throw Object.defineProperty(Error(`Invariant: app-route received invalid cache entry ${null==u||null==(c=u.value)?void 0:c.kind}`),"__NEXT_ERROR_CODE",{value:"E701",enumerable:!1,configurable:!0});l||t.setHeader("x-nextjs-cache",x?"REVALIDATED":u.isMiss?"MISS":u.isStale?"STALE":"HIT"),E&&t.setHeader("Cache-Control","private, no-cache, no-store, max-age=0, must-revalidate");let h=(0,f.fromNodeOutgoingHttpHeaders)(u.value.headers);return l&&$||h.delete(b.NEXT_CACHE_TAGS_HEADER),!u.cacheControl||t.getHeader("Cache-Control")||h.get("Cache-Control")||h.set("Cache-Control",(0,y.getCacheControlHeader)(u.cacheControl)),await (0,g.sendResponse)(q,U,new Response(u.value.body,{headers:h,status:u.value.status||200})),null};Y?await c(Y):await z.withPropagatedContext(e.headers,()=>z.trace(m.BaseServerSpan.handleRequest,{spanName:`${F} ${a}`,kind:n.SpanKind.SERVER,attributes:{"http.method":F,"http.target":e.url}},c))}catch(t){if(t instanceof v.NoFallbackError||await k.onRequestError(e,t,{routerKind:"App Router",routePath:P,routeType:"route",revalidateReason:(0,p.getRevalidateReason)({isStaticGeneration:H,isOnDemandRevalidate:x})}),$)throw t;return await (0,g.sendResponse)(q,U,new Response(null,{status:500})),null}}e.s(["handler",()=>A,"patchFetch",()=>R,"routeModule",()=>k,"serverHooks",()=>O,"workAsyncStorage",()=>E,"workUnitAsyncStorage",()=>C]),o()}catch(e){o(e)}},!1)];
